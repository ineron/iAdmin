...
public:
	...
	{
		path_to_latex_ = config.get("admin.tex.latex","/usr/bin/latex");
		path_to_dvigif_ = config.get("admin.tex.dvigif","/usr/bin/dvigif");
		path_to_tmp_ = config.get("admin.tex.temp_dir","/tmp");
		std::string tmp_www_dir = config.get<std::string>("admin.media") + "/tex";
		www_dir_ = config.get("admin.tex.www_root",tmp_www_dir);
		output_dir_ = config.get("admin.tex.output_dir","");
...
namespace apps {
	...
	void BasicMaster::clear()
	{
		sql_->close();
	}
    void BasicMaster::prepare(data::admin::basic_master &c)
	{
		...
		r=sql()<<"SELECT id,value FROM global.text_options ";
    ...
    void BasicMaster::prepare_nav(data::admin::basic_master &c)
    {
         ...
         */
        if(cache().fetch_data("nav_items",c.navItems))
            return;
         ...
         select row_to_json(t) from (
            SELECT array_to_json(array_agg(row_to_json(t))) records FROM(
                SELECT ref, parent_ref, order_id, level_id, name,
                    href, link_type, CASE WHEN icon is null THEN '' ELSE icon END icon,case when badge is null then '' else badge end badge,0 acl,'menu-item-parent' style,
                        (SELECT array_to_json(array_agg(row_to_json(t))) from (SELECT ref, parent_ref, order_id, level_id, name,href, link_type, CASE WHEN icon is null THEN '' ELSE icon END icon,case when badge is null then '' else badge end badge,0 acl,'menu-item-parent' style,null child
                        FROM global.nav_items where parent_ref = n2.ref)t) child
        ...
        if (!object.is_undefined()) {
            ...
            for(cppcms::json::array::const_iterator i = records.begin();i!=records.end();i++) {
                data::admin::NavItem item;
                item = (*i).get_value<data::admin::NavItem>();
                c.navItems.push_back(item);
            }
        }
        
        cache().store_data("nav_items",c.navItems);
    
    }
	BasicMaster::BasicMaster(cppcms::service &s) : cppcms::application(s)
	{
		media_ = settings().get<std::string>("admin.media");
		cookie_ = settings().get<std::string>("session.cookies.prefix","cppcms_session");
		host_ = settings().get<std::string>("admin.host");
		conn_str_ = settings().get<std::string>("admin.connection_string");
